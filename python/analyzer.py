"""Claude AI로 구글 캘린더(계획) vs Notion Inbox(실행) 비교 분析."""
import json
...
try:
    from criteria import get_habits, get_analysis_perspectives, get_analysis_style, get_grade_criteria
    _HABITS = get_habits()
    _PERSPECTIVES = get_analysis_perspectives()
    _STYLE = get_analysis_style()
    _GRADES = get_grade_criteria()
except Exception:
    _HABITS, _PERSPECTIVES, _STYLE, _GRADES = [], [], {}, []

client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)

SYSTEM_PROMPT = """당신은 개인 생산성 코치입니다.
사용자의 구글 캘린더(계획)와 노션 Inbox(실행 캡처)를 비교 분析하여 인사이트를 제공합니다.
...
_JSON_TEMPLATE = """{
  ...
  "metrics": {
    "total_calendar_events": __TOTAL_CAL__,
    "total_inbox_items": __TOTAL_INBOX__,
    "inbox_process_rate": __PROCESS_RATE__,
...
def _extra_context() -> str:
    """분析기준.txt에서 로드한 추가 컨텍스트를 반환."""
    parts = []
    if _HABITS:
        parts.append(
            '## 추적 습관 목록\n' + ', '.join(_HABITS)
            + '\n위 습관들이 이번 주 Inbox에 기록되었는지 확인하고 분析하세요.'
        )
    if _PERSPECTIVES:
        parts.append('## 추가 분析 관점\n' + '\n'.join(f'- {p}' for p in _PERSPECTIVES))
    if _GRADES:
        grade_str = ', '.join(f'{g}={p}%이상' for g, p in _GRADES)
        parts.append(f'## Inbox 처리율 등급 기준\n{grade_str}')
    if _STYLE:
        parts.append(
            '## 작성 스타일\n'
            f'- 말투: {_STYLE.get("말투", "")}\n'
...
def _build_prompt(week: dict, cal_by_date: dict, inbox_items: list[dict], inbox_summary: dict) -> str:
    ...
    json_template = (
        _JSON_TEMPLATE
        .replace('__TOTAL_CAL__', str(total_cal))
        .replace('__TOTAL_INBOX__', str(inbox_summary['total']))
        .replace('__PROCESS_RATE__', str(inbox_summary['process_rate']))
    )

    return (
        '아래 데이터를 분析하여 JSON으로 출력하세요.\n\n'
        f'## 분析 기간\n{monday} (월) ~ {sunday} (일)\n\n'
        f'## 구글 캘린더 일정 (계획, 총 {total_cal}건)\n{cal_section}\n\n'
        f'## Notion Inbox 항목 (캡처/실행, 총 {inbox_summary["total"]}건)\n'
        f'- 처리 완료: {inbox_summary["processed"]}건 ({inbox_summary["process_rate"]}%)\n'
        f'- 미처리: {inbox_summary["unprocessed"]}건\n'
        f'- 출처별: {json.dumps(inbox_summary["by_source"], ensure_ascii=False)}\n'
        f'{inbox_section}'
        f'{extra_block}\n\n'
        '## 분析 지침\n'
...
def analyze(week: dict, cal_by_date: dict, inbox_items: list[dict], inbox_summary: dict) -> dict:
    ...
    try:
        ...
        print(f'[AI 分析] JSON 파싱 실패 ({exc}), 원본 앞 500자:\n{raw[:500]}')
